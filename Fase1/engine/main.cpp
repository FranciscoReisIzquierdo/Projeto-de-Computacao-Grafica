...
void render_object(std::list<object>::iterator oit){
	...
	glPushMatrix();
	// Position
	glTranslatef(oit->position.x, oit->position.y, oit->position.z);
	// Rotation
	glRotatef(oit->rotation.x, 1, 0, 0);
	glRotatef(oit->rotation.y, 0, 1, 0);
	glRotatef(oit->rotation.z, 0, 0, 1);
	// Scale
	glScalef(oit->scale.x, oit->scale.y, oit->scale.z);
...
void draw_ui(){
	...
	char str1[100] = "Change Object => AWSD";
...
void read_object(std::string path, point pos, point rot, point scale, float r, float g, float b, bool centerable){
...
void read_group(TiXmlElement* group, point pos, point rot, point scale){

	TiXmlElement* son = group->FirstChildElement();
	point new_pos, new_rot, new_scale;
	new_pos.x = pos.x;
	new_pos.y = pos.y;
	new_pos.z = pos.z;
	new_rot.x = rot.x;
	...
	for (; son!=nullptr; son = son->NextSiblingElement()){
		if (strcmp(son->Value(), "transform") == 0){
			TiXmlElement* position = son->FirstChildElement();
			new_pos.x = pos.x + atof(position->Attribute("x"));
			new_pos.y = pos.y + atof(position->Attribute("y"));
			new_pos.z = pos.z + atof(position->Attribute("z"));

			TiXmlElement* rotation = position->NextSiblingElement();
			int x = atoi(rotation->Attribute("x"));
			int y = atoi(rotation->Attribute("y"));
			int z = atoi(rotation->Attribute("z"));
			float angle = atof(rotation->Attribute("angle"));
			if (x == 1)
				new_rot.x = rot.x + angle;
			if (y == 1)
				new_rot.y = rot.y + angle;
			if (z == 1)
				new_rot.z = rot.z + angle;

			TiXmlElement* sc = rotation->NextSiblingElement();
			new_scale.x = scale.x * atof(sc->Attribute("x"));
			new_scale.y = scale.y * atof(sc->Attribute("y"));
			new_scale.z = scale.z * atof(sc->Attribute("z"));
		}
		else if (strcmp(son->Value(), "group") == 0){
			read_group(son, new_pos, new_rot, new_scale);
		}
			...
			for (; model!=nullptr; model = model->NextSiblingElement()){
				...
				read_object(model->Attribute("file"), new_pos, new_rot, new_scale, r, g, b, centerable);
			}
		}
	}
}
...
bool init_engine(char* file_path){
	...
	scale.y = 1;
	scale.z = 1;
	read_group(main_group, position, rotation, scale);
/*
	for (; model!=nullptr; model = model->NextSiblingElement()) {
		read_object(model->Attribute("file"));
	}*/
...
int main(int argc, char** argv)
{
	...
	glClearColor(0, 0, 0.1, 1);//0.5f, 0.5f, 0.5f, 0.5f);
